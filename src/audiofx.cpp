...
namespace jimmbot_audiofx
{
  ...
  {
    this->extn_data_sub_ = nh->subscribe<jimmbot_msgs::extn_data>("extn_data", 1, &AudioFX::extnDataMsgCallback, this);
    this->audio_buffer_pub_ = nh->advertise<audio_common_msgs::AudioData>("audio", 1, false);
  }
  
  void AudioFX::extnDataMsgCallback(const jimmbot_msgs::extn_data::ConstPtr &extn_data_msg)
  {
    if(extn_data_msg->horn && !is_called_)
    {
    ...
    else if(!extn_data_msg->horn && is_called_)
    {
